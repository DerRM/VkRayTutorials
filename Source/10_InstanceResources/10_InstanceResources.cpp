...
{
    _appName = "VkRay Tutorial 10: Instance resources";
...
void TutorialApplication::CreatePipeline()
{
...

    {
        ...
        LoadShader(rgenShader, "rt_10_shaders.rgen.spv");
        LoadShader(missShader, "rt_10_shaders.rmiss.spv");
        LoadShader(chitShader, "rt_10_shaders.rchit.spv");
...
void TutorialApplication::CreateUniformBuffers()
{
    ...
    for (uint32_t i = 0; i < _uniformBuffers.size(); i++)
    {
        ...
        {
            ExitError("Failed to copy uniform buffer");
        }
    }
}
