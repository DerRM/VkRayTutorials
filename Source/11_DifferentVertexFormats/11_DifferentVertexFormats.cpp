...
public:
    ...
    void CreateBox(RenderObject& object, const std::string& texturePath);
    ...
    void LoadObjectTexture(RenderObject& object, const std::string& path);
...
{
    _appName = "VkRay Tutorial 11: Different Vertex Formats";
...
void TutorialApplication::Init()
{
    ...
    CreateBox(_renderObjects[0], "cb0.bmp");
    CreateIcosahedron(_renderObjects[1]);
    CreateBox(_renderObjects[2], "cb1.bmp");
    CreateBox(_renderObjects[3], "cb2.bmp");
...
void CreateBufferAndUploadData( BufferResource& buffer, VkBufferUsageFlags usage, const std::vector<T>& content )
{
    ...
    NVVK_CHECK_ERROR(code, "rt BufferResource::Create");

    if (!buffer.CopyToBufferUsingMapUnmap(content.data(), size))
    {
        ExitError("Failed to copy data to buffer");
    }
}
...
void TutorialApplication::CreateBox(RenderObject& object, const std::string& texturePath)
{
...
void TutorialApplication::CreateIcosahedronBufferViews(RenderObject& object)
{
    ...
    NVVK_CHECK_ERROR(code, "vkCreateBufferView");
...
void TutorialApplication::LoadObjectTexture(RenderObject& object, const std::string& path)
{
    VkResult code;
    if (!object.texture.LoadTexture2DFromFile(path, code))
    {
        ExitError("Failed to load texture. VkResult: " + std::to_string(code));
    }
    ...
    NVVK_CHECK_ERROR(code, "Failed to create image view.");
    ...
    NVVK_CHECK_ERROR(code, "Failed to create sampler.");
...
{
    {
        ...
        NVVK_CHECK_ERROR(code, "vkCreateDescriptorSetLayout");
    }
...
void TutorialApplication::CreatePipeline()
{
    ...
    LoadShader(rgenShader, "rt_11_shaders.rgen.spv");
    LoadShader(missShader, "rt_11_shaders.rmiss.spv");
    LoadShader(chitShaders[0], "rt_11_box.rchit.spv");
    LoadShader(chitShaders[1], "rt_11_icosahedron.rchit.spv");
