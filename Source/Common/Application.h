...
void LogError(const std::string& message, bool silent = false);
void ExitError(const std::string& message, bool silent = false);
...
public:
    ...
    bool LoadTexture2DFromFile(const std::string& fileName, VkResult& vkResult);
...
#define NVVK_RESOLVE_INSTANCE_FUNCTION_ADDRESS(instance, funcName) \
    { \
        ...
        { \
            ...
            ExitError(std::string("Can't get instance function address: ") + std::string(name.begin(), name.end())); \
        } \
    }
...
#define NVVK_RESOLVE_DEVICE_FUNCTION_ADDRESS(device, funcName) \
    { \
        ...
        { \
            const std::string name = #funcName; \
            ExitError(std::string("Can't get device function address: ") + std::string(name.begin(), name.end())); \
        } \
    }
...
#define NVVK_CHECK_ERROR(code, message) \
    { \
        if (code != VK_SUCCESS) \
        { \
            ExitError(message + std::string(" ErrorCode: ") + ToString(code)); \
        } \
    }
