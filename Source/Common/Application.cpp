...
void LogError(const std::string& message, bool silent)
{
    if (!silent)
    {
...
void ExitError(const std::string& message, bool silent)
{
...
std::string ShaderResource::_folderPath;
std::string ImageResource::_folderPath;

Application* Application::_applicationInstance = nullptr;


Application::Application() : _appName("Vk tutorial")
{
...
void Application::Loop()
{
    ...
    while (!quitMessageReceived)
    {
        ...
        while ((event = xcb_poll_for_event(_windowInfo.connection)) != NULL)
        {
			switch (event->response_type & ~0x80) {
				case XCB_CLIENT_MESSAGE: {
					xcb_client_message_event_t *cm = reinterpret_cast<xcb_client_message_event_t*>(event);
				    if (cm->data.data32[0] == _windowInfo.wmDeleteWin)
				    {
...
void Application::InitCommon()
{
    ...
    _basePath = std::string(dest);
    ShaderResource::SetFolderPath(_basePath + "/Assets/Shaders/");
    ImageResource::SetFolderPath(_basePath + "/Assets/Textures/");
}
...
void Application::CreateApplicationWindow()
{
	...
	int screenp = 0;
	_windowInfo.connection = xcb_connect(NULL, &screenp);
	if (xcb_connection_has_error(_windowInfo.connection)) {
		ExitError("Failed to connect to X server using XCB");	
	}

	xcb_screen_iterator_t iter = xcb_setup_roots_iterator(xcb_get_setup(_windowInfo.connection));
	...
	_windowInfo.window = xcb_generate_id(_windowInfo.connection);

	uint32_t eventMask = XCB_CW_BACK_PIXEL | XCB_CW_EVENT_MASK;
	uint32_t valueList[] = { screen->black_pixel, 0 };

	xcb_create_window(_windowInfo.connection, XCB_COPY_FROM_PARENT, _windowInfo.window, screen->root, 0, 0, _settings.DesiredWindowWidth, _settings.DesiredWindowHeight, 0, XCB_WINDOW_CLASS_INPUT_OUTPUT, screen->root_visual, eventMask, valueList);

    _actualWindowWidth = _settings.DesiredWindowWidth;
    _actualWindowHeight = _settings.DesiredWindowHeight;

    xcb_change_property(_windowInfo.connection, XCB_PROP_MODE_REPLACE, _windowInfo.window, XCB_ATOM_WM_NAME, XCB_ATOM_STRING, 8, static_cast<uint32_t>(_appName.size()), _appName.c_str());


	xcb_intern_atom_cookie_t wmDeleteCookie = xcb_intern_atom(_windowInfo.connection, 0, strlen("WM_DELETE_WINDOW"), "WM_DELETE_WINDOW");
    xcb_intern_atom_cookie_t wmProtocolsCookie = xcb_intern_atom(_windowInfo.connection, 0, strlen("WM_PROTOCOLS"), "WM_PROTOCOLS");
    xcb_intern_atom_reply_t *wmDeleteReply = xcb_intern_atom_reply(_windowInfo.connection, wmDeleteCookie, nullptr);
    xcb_intern_atom_reply_t *wmProtocolReply = xcb_intern_atom_reply(_windowInfo.connection, wmProtocolsCookie, nullptr);

    _windowInfo.wmDeleteWin = wmDeleteReply->atom;
    xcb_change_property(_windowInfo.connection, XCB_PROP_MODE_REPLACE, _windowInfo.window, wmProtocolReply->atom, 4, 32, 1, &wmDeleteReply->atom);

    xcb_map_window(_windowInfo.connection, _windowInfo.window);
    xcb_flush(_windowInfo.connection);
...
void Application::CreateInstance()
{
    ...
    NVVK_CHECK_ERROR(code, "vkCreateInstance");
}
...
void Application::FindDeviceAndQueues()
{
    ...
    {
        ExitError("No physical device found");
    }
...
void Application::CreateDevice()
{
    ...
    NVVK_CHECK_ERROR(code, "vkCreateDevice");
}
...
void Application::CreateDebugReportCallback()
{
    ...
    NVVK_CHECK_ERROR(code, "vkCreateDebugReportCallbackEXT");
}
...
void Application::CreateSurface()
{
    ...
    surfaceCreateInfo.connection = _windowInfo.connection;
    surfaceCreateInfo.window = _windowInfo.window;

    VkResult code = vkCreateXcbSurfaceKHR(_instance, &surfaceCreateInfo, nullptr, &_surface);
    NVVK_CHECK_ERROR(code, "vkCreateXcbSurfaceKHR");
    ...
    NVVK_CHECK_ERROR(code, "vkGetPhysicalDeviceSurfaceSupportKHR");

    if (!supportPresent)
    {
        ExitError("Graphics queue does not support presenting");
    }
...
void Application::CreateSwapchain()
{
    ...
    NVVK_CHECK_ERROR(code, "vkGetPhysicalDeviceSurfaceCapabilitiesKHR");
    ...
    NVVK_CHECK_ERROR(code, "vkCreateSwapchainKHR");
    ...
    for (uint32_t i = 0; i < imageCount; i++)
    {
        ...
        NVVK_CHECK_ERROR(code, "vkCreateImageView " + std::to_string(i))
    }
}
...
void Application::CreateOffsreenBuffers()
{
    ...
    NVVK_CHECK_ERROR(code, "_offsreenImageResource.CreateImage");

    code = _offsreenImageResource.CreateImageView(VK_IMAGE_VIEW_TYPE_2D, _surfaceFormat.format,
        { VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1 });

    NVVK_CHECK_ERROR(code, "_offsreenImageResource.CreateImageView");
}
...
void Application::CreateCommandPool()
{
    ...
    NVVK_CHECK_ERROR(code, "vkCreateCommandPool");
}
...
void Application::CreateCommandBuffers()
{
    ...
    NVVK_CHECK_ERROR(code, "vkAllocateCommandBuffers");
}
...
void Application::CreateSynchronization()
{
    ...
    NVVK_CHECK_ERROR(code, "vkCreateSemaphore");
...
void Application::FillCommandBuffers()
{
    ...
    for (uint32_t i = 0; i < _commandBuffers.size(); i++)
    {
        ...
        NVVK_CHECK_ERROR(code, "vkBeginCommandBuffer");
        ...
        NVVK_CHECK_ERROR(code, "vkEndCommandBuffer");
    }
}
...
void Application::DrawFrame()
{
    ...
    NVVK_CHECK_ERROR(code, "vkQueueSubmit");
    ...
    NVVK_CHECK_ERROR(code, "vkQueuePresentKHR");
}
...
void ImageResource::SetFolderPath(const std::string& folderPath)
{
...
bool ImageResource::LoadTexture2DFromFile(const std::string& fileName, VkResult& code)
{
    code = VK_SUCCESS;

    const std::string filePath = _folderPath + fileName;
    FILE *file;
    if ((file = fopen(filePath.c_str(), "rb")) == nullptr)
    {
        return false;
    }
...
void ShaderResource::SetFolderPath(const std::string& folderPath)
{
    _folderPath = folderPath;
}
...
void* BufferResource::Map(VkDeviceSize size) const
{
    ...
    if (code != VK_SUCCESS)
    {
        LogError("vkMapMemory: " + ToString(code));
        return nullptr;
    }
    return mappedMemory;
}
